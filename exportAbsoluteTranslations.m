function [positions] = exportAbsoluteTranslations(theta, joints, cluster)
    for i = 1:max(cluster)
        if (size(theta(i).rx,1) > 0) ...
           ...
           positions(:,i,1) = joints(cluster(i)).abs_pos(1)*ones(size(theta(1).rx));
           positions(:,i,2) = joints(cluster(i)).abs_pos(2)*ones(size(theta(1).ry));
           positions(:,i,3) = joints(cluster(i)).abs_pos(3)*ones(size(theta(1).rz)); 
        end
    end
