function [clusters, joints, curves] = xcorrCluster(data, k)
...
for i = 1:msize
    ...
    joints(i,4,:) = data(i).t_xyz(1,:);
    joints(i,5,:) = data(i).t_xyz(2,:);
    joints(i,6,:) = data(i).t_xyz(3,:);
end
...
while (swapped)
    ...
    for i = 1:msize
        max = 0;
        for j = 1:k
            diff = xcorrDist(centroids(j,:,:), joints(i, :, :));
